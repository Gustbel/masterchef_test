...
async function main() {
    ...
    await lp2Token.connect(owner).transfer(carol.address, 20)
    ...
    await lp2Token.connect(carol).approve(chef.address, 20)
    depositionPoint = await chef.connect(carol).deposit(1, 20, carol.address)
