...
async function main() {
    ...
    const lp2Token = await Lp2Token.deploy("Lp2Token", "LP2", 0, 1000);
    ...
    console.log ("\t - Sending 10 LP1 to Alice, 20 LP2 to Bob and 20 LP2 to Carol ...");
    await lp1Token.connect(owner).transfer(alice.address, 10)
    await lp2Token.connect(owner).transfer(bob.address, 20)
    await lp2Token.connect(owner).transfer(carol.address, 200)
    ...
    // Carol make a deposit
    console.log (`\t - Carol Deposit 20 tokens LP2 in Masterchef at ` + tx.blockNumber + ` Block ...`);
    await lp2Token.connect(carol).approve(chef.address, 200)
    depositionPoint = await chef.connect(carol).deposit(1, 200, carol.address)

    tx = await chef.connect(owner).updatePool(0)
    alicePendingPools = await chef.pendingBeets(0, alice.address)
    bobPendingPools = await chef.pendingBeets(1, bob.address)
    let carolPendingPools = await chef.pendingBeets(1, carol.address)
    console.log (`\t\t In Block ` + tx.blockNumber + `. Alice has ${alicePendingPools/1e18} pending pools, Bob has ${bobPendingPools/1e18} pending pools and Carol has ${carolPendingPools/1e18} pending pools`);
    ...
    let carolPendingPools_1block = await chef.pendingBeets(1, carol.address)
    console.log (`\t\t In Block ` + tx.blockNumber + `. Alice has ${alicePendingPools_1block/1e18} pending pools and Bob has ${bobPendingPools_1block/1e18} pending pools`);

    console.log("\n\tNOW ALICE GENERATE A REWARD OF: " + (alicePendingPools_1block/1e18 - alicePendingPools/1e18) + " POOLS per block");
    console.log("\t and BOB GENERATE A REWARD OF: " + (bobPendingPools_1block/1e18 - bobPendingPools/1e18) + " POOLS per block");
    console.log("\t CAROL GENERATE A REWARD OF: " + (carolPendingPools_1block/1e18 - carolPendingPools/1e18) + " POOLS per block\n");
    ...
    let carol_pools_balance = await pools.balanceOf(carol.address)
    console.log (`\t\t POOLS Tokens balance in CAROL wallet (before the harvest): ${carol_pools_balance/1e18}`);
    ...
    carol_pools_balance = await pools.balanceOf(carol.address)
    console.log (`\t\t POOLS Tokens balance in CAROL wallet (AFTER the harvest): ${carol_pools_balance/1e18}`);
